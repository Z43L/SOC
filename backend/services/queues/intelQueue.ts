...
import { IntelReport, virustotalProvider, abuseipdbProvider, otxProvider, osvProvider } from '../threatIntel';
...
new Worker('intel', async job => {
  const { ioc, type } = job.data as { ioc: string; type: string };
  // Check cache for valid entry
  const cacheHits = await db.select().from(threatIntelCache)
    .where(and(
      eq(threatIntelCache.iocValue, ioc),
      sql`(last_seen + ttl) > now()`
    ))
    .execute();
