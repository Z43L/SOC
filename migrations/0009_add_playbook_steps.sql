...
-- Note: This assumes that JSON objects in playbooks.steps have keys: id (step_key), uses (action name), if (condition), with (inputs), onError, timeout
...
SELECT
  ...
  elem.value->'with' AS inputs,
  COALESCE(elem.value->>'onError','abort') AS on_error,
  (elem.value->>'timeout')::integer AS timeout_ms
FROM playbooks p,
LATERAL jsonb_array_elements(p.steps) WITH ORDINALITY AS elem(value, idx)
LEFT JOIN actions a ON a.name = elem.value->>'uses';

-- Drop old steps column if all data migrated
ALTER TABLE playbooks DROP COLUMN IF EXISTS steps;
