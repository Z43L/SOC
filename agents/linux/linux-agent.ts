...
export class LinuxAgent extends AgentBase {
  ...
  private async checkSuspiciousProcess(process: Monitoring.ProcessInfo): Promise<void> {
    ...
    // Procesos con alta utilización de CPU
    if (process.cpuUsage > 90) {
      await this.queueEvent({
        eventType: 'process',
        severity: 'medium',
        timestamp: new Date(),
  ...
  protected async scanFileSystem(): Promise<void> {
    console.log('Scanning file system for suspicious files');
  ...
  private async getFileHash(filePath: string): Promise<string | null> {
    try {
      // En una implementación real, calcularíamos el hash SHA-256 del archivo
      // Aquí devolvemos un valor simulado
