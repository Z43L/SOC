...
export class MacOSAgent extends AgentBase {
  ...
  private async checkProcesses(): Promise<void> {
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        const user = parts[1];
        // El resto es el comando
        const command = parts.slice(2).join(' ');
        
        // Obtener nombre base del proceso
        const processName = path.basename(command.split(' ')[0]);
  ...
  private async isSuspiciousProcess(process: Monitoring.ProcessInfo): Promise<boolean> {
    ...
    for (const pattern of suspiciousPatterns) {
      if (process.cmdline.includes(pattern) || 
          new RegExp(pattern).test(process.cmdline)) {
        return true;
      }
    }
  ...
  private async checkNetworkConnections(): Promise<void> {
    try {
      ...
      for (let i = 1; i < lines.length; i++) { // Saltar cabecera
        ...
        const protocol = parts[8].includes('UDP') ? 'UDP' : 'TCP';
        
        // Extraer IP y puerto de la última columna
        ...
        if (addressInfo.includes('->')) {
          ...
          if (remotePart.includes(':')) {
            ...
            // El estado puede estar después del puerto
            if (portAndState.includes('(')) {
  ...
  private async isSuspiciousConnection(connection: Monitoring.NetworkConnection): Promise<boolean> {
    ...
    if (connection.state === 'ESTABLISHED' &&
        suspiciousProcesses.some(proc => connection.processName.toLowerCase() === proc.toLowerCase())) {
      return true;
    }
  ...
  protected async scanFileSystem(): Promise<void> {
    ...
    const expandedDirs = this.suspiciousDirectories.map(dir => 
