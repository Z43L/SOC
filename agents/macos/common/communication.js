...
class AgentCommunication {
     ...
     */
    async registerAgent(hostname, ip, os, version, capabilities) {
        try {
            const payload = {
                hostname,
                ip,
            ...
            const response = await this.sendRequest('POST', this.config.registrationEndpoint, payload);
            if (!response.success) {
                return {
                    success: false,
    ...
    async sendEvents(events) {
        try {
            ...
            const response = await this.sendRequest('POST', this.config.dataEndpoint, payload);
            return {
     ...
     */
    async sendHeartbeat(status, metrics) {
        try {
            if (!this.config.agentId) {
                return {
                    success: false,
            ...
            const response = await this.sendRequest('POST', this.config.heartbeatEndpoint, payload);
            return {
                success: response.success,
                message: response.message,
                config: response.data?.config || {}
