...
export class ReportGeneratorService {
    ...
    registerHandlebarsHelpers() {
        ...
        // Percentage helper
        Handlebars.registerHelper('percentage', (value, decimals = 1) => {
            return `${(value * 100).toFixed(decimals)}%`;
        });
        // Risk level helper
        Handlebars.registerHelper('riskLevel', (score) => {
            if (score >= 80)
                return 'High Risk';
            if (score >= 60)
                return 'Medium Risk';
        ...
        // Risk color helper
        Handlebars.registerHelper('riskColor', (score) => {
            if (score >= 80)
    ...
    async checkScheduledReports() {
        try {
            ...
            const templates = await this.storage.db
                ...
                .where(this.storage.db.and(this.storage.db.eq(this.storage.schema.reportTemplates.isEnabled, true), this.storage.db.isNotNull(this.storage.schema.reportTemplates.scheduleCron)));
    ...
    async shouldExecuteTemplate(template) {
    ...
    async getLastReportGeneration(templateId) {
    ...
    async generateReportFromTemplate(templateId, requestedBy, trigger = 'manual') {
        const template = await this.getReportTemplate(templateId);
        if (!template) {
            throw new Error('Report template not found');
        }
        // Calculate report period based on template parameters
        const { periodFrom, periodTo } = this.calculateReportPeriod(template.parameters);
    ...
    calculateReportPeriod(parameters) {
        const now = new Date();
    ...
    async generateReportContent(template, periodFrom, periodTo) {
    ...
    async generateExecutiveSummaryContent(reportData, template, periodFrom, periodTo) {
    ...
    async generateTechnicalIncidentsContent(reportData, template, periodFrom, periodTo) {
        const orgId = template.organizationId;
        const parameters = template.parameters;
        const incidents = await this.storage.listIncidents(orgId, 1000, 0);
        const severityFilter = parameters.severityFilter || ['critical', 'high', 'medium', 'low'];
        const filteredIncidents = incidents.filter(i => severityFilter.includes(i.severity) &&
    ...
    async generateComplianceAuditContent(reportData, template, periodFrom, periodTo) {
        const orgId = template.organizationId;
    ...
    async generateAgentHealthContent(reportData, template, periodFrom, periodTo) {
        const orgId = template.organizationId;
    ...
    async generateVulnerabilityAssessmentContent(reportData, template, periodFrom, periodTo) {
    ...
    async generateThreatIntelligenceContent(reportData, template, periodFrom, periodTo) {
        const orgId = template.organizationId;
    ...
    async generateSOCPerformanceContent(reportData, template, periodFrom, periodTo) {
        const orgId = template.organizationId;
        const alerts = await this.storage.listAlerts(orgId, 1000, 0, periodFrom, periodTo);
        const incidents = await this.storage.listIncidents(orgId, 1000, 0);
        const metrics = await this.storage.listMetrics(orgId);
    ...
    async generateCustomContent(reportData, template, periodFrom, periodTo) {
        const parameters = template.parameters;
        reportData.sections.push({
            title: 'Custom Report',
            type: 'custom',
    ...
    calculateRiskScore(alerts, incidents) {
    ...
    calculateTrend(alerts) {
        ...
        if (recentAlerts > olderAlerts * 1.1)
            return 'up';
        if (recentAlerts < olderAlerts * 0.9)
            return 'down';
        return 'stable';
    }
    groupBy(items, key) {
    ...
    calculateOverallCompliance(assessments) {
    ...
    identifyComplianceGaps(assessments) {
        return assessments
    ...
    calculateAnalystPerformance(alerts, incidents) {
        return {
    ...
    async generatePDF(report, reportData) {
    ...
    async sendEmailNotification(template, report, filePath) {
        if (!this.emailTransporter || !template.notifyEmails)
            return;
        const emails = template.notifyEmails;
        const organization = await this.storage.getOrganization(template.organizationId);
        const mailOptions = {
    ...
    async listReports(organizationId, limit = 50, offset = 0) {
    ...
    generateSignedUrl(filePath, expiresIn = 600) {
