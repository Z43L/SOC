...
const requireOrgAccess = async (req: Request, res: Response, next: Function) => {
  try {
 ...
 */
router.post('/', isAuthenticated, requireRole('ADMIN'), async (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/:id', isAuthenticated, requireOrgAccess, async (req: Request, res: Response) => {
  try {
 ...
 */
router.patch('/:id', isAuthenticated, requireRole('ADMIN'), requireOrgAccess, async (req: Request, res: Response) => {
  try {
    ...
    if (Object.keys(configUpdates).length > 0 || credentials) {
      const updateConfig: any = { ...configUpdates };
 ...
 * Elimina un conector
 */
router.delete('/:id', isAuthenticated, requireRole('ADMIN'), requireOrgAccess, async (req: Request, res: Response) => {
  try {
 ...
 */
router.post('/:id/test', isAuthenticated, requireOrgAccess, async (req: Request, res: Response) => {
  try {
    const activeConnector = connectorManager.getConnector(req.params.id);
 ...
 */
router.get('/schema/:type', isAuthenticated, async (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/:id/logs', isAuthenticated, requireOrgAccess, async (req: Request, res: Response) => {
  try {
