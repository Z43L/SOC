...
// Change password
router.post('/user/password', isAuthenticated, validateRequest(PasswordChangeSchema), async (req: Request, res: Response) => {
  try {
...
// Upload avatar
router.post('/user/avatar', isAuthenticated, upload.single('avatar'), async (req: Request, res: Response) => {
  try {
    if (!req.file) {
...
router.post('/user/mfa/enable', isAuthenticated, async (req: Request, res: Response) => {
  try {
...
router.post('/user/mfa/verify', isAuthenticated, validateRequest(MFAVerifySchema), async (req: Request, res: Response) => {
  try {
    const { token } = req.body;
    const verified = await UserSettingsService.verifyAndActivateMFA(
      req.user!.id,
      token,
      req.user!.organizationId
    );
...
// Disable MFA
router.post('/user/mfa/disable', isAuthenticated, async (req: Request, res: Response) => {
  try {
...
router.get('/org', isAuthenticated, requireRole('admin'), async (req: Request, res: Response) => {
  try {
...
// Test integrations
router.post('/org/test-integration', isAuthenticated, requireRole('admin'), validateRequest(TestIntegrationSchema), async (req: Request, res: Response) => {
  try {
...
router.post('/org/logo', isAuthenticated, requireRole('admin'), upload.single('logo'), async (req: Request, res: Response) => {
  try {
    if (!req.file) {
      return res.status(400).json({ error: 'No file uploaded' });
    }
...
router.get('/user/history', isAuthenticated, async (req: Request, res: Response) => {
  try {
...
router.get('/org/history', isAuthenticated, requireRole('admin'), async (req: Request, res: Response) => {
  try {
    const limit = parseInt(req.query.limit as string) || 50;
...
router.get('/timezones', isAuthenticated, async (req: Request, res: Response) => {
  try {
...
router.get('/locales', isAuthenticated, async (req: Request, res: Response) => {
  try {
    const locales = [
