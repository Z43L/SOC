...
export class PlaybookTriggerEngine {
    ...
    consumerGroup = 'playbook-trigger-engine';
    constructor() {
    ...
    async startConsumingEvents() {
        try {
            // Read new messages continuously
            while (true) {
                try {
                    // Read from stream with a blocking call (wait for 2 seconds)
                    const results = await this.redisClient.xreadgroup('GROUP', this.consumerGroup, 'consumer1', 'COUNT', '10', 'BLOCK', '2000', 'STREAMS', this.streamName, '>');
    ...
    async publishEventToStream(event) {
        try {
            ...
            // Add event to stream
            const messageId = await this.redisClient.xadd(this.streamName, '*', // Auto-generate ID
            'id', event.id, 'type', event.type, 'entityId', event.entityId.toString(), 'entityType', event.entityType, 'organizationId', event.organizationId.toString(), 'timestamp', event.timestamp || new Date().toISOString(), 'data', serializedData);
    ...
    async processEvent(job) {
        ...
        try {
            ...
            const matchingBindings = await db.select()
                ...
                .where(and(eq(playbookBindings.eventType, event.type), eq(playbookBindings.isActive, true), eq(playbookBindings.organizationId, event.organizationId)))
            ...
            for (const binding of matchingBindings) {
                if (await this.evaluateBinding(binding, event)) {
                    ...
                    const [playbook] = await db.select()
                        .from(playbooks)
                        .where(and(eq(playbooks.id, binding.playbookId), eq(playbooks.isActive, true)));
                    if (playbook) {
    ...
    async evaluateBinding(binding, event) {
        try {
            ...
            if (predicate.includes('tags.contains')) {
                ...
                if (tagMatch && tagMatch[1]) {
                    ...
                    return tags.some((tag) => tag.toLowerCase() === expectedTag);
                }
            }
