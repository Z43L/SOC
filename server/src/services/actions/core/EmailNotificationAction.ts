...
export class EmailNotificationAction extends BaseAction {
  readonly name = 'notify_email';
  readonly description = 'Send email notification';
  ...
  async execute(params: Record<string, any>, context: ActionContext): Promise<ActionResult> {
    try {
      ...
      return this.success(
        `Email sent successfully to ${emailData.to.join(', ')}`,
        {
  ...
  private async sendEmail(emailData: any, context: ActionContext): Promise<void> {
