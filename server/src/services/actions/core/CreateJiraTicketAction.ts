...
export class CreateJiraTicketAction extends BaseAction {
  readonly name = 'create_jira_ticket';
  readonly description = 'Create Jira ticket';
  ...
  async execute(params: Record<string, any>, context: ActionContext): Promise<ActionResult> {
    try {
      ...
      this.log(context, 
        `Jira ticket created successfully: ${ticketResult.key}. ` +
        `URL: ${ticketResult.url}`
      );

      return this.success(
        `Jira ticket ${ticketResult.key} created successfully`,
        {
  ...
  private async createJiraTicket(params: any, context: ActionContext): Promise<{ key: string; id: string; url: string }> {
    ...
    const issuePayload = {
      fields: {
        ...
        ...(params.components && { components: params.components.map((c: string) => ({ name: c })) }),
  ...
  private formatDescriptionForJira(description: string, context: ActionContext): string {
  ...
  private async addWatchers(ticketKey: string, watchers: string[], context: ActionContext): Promise<void> {
