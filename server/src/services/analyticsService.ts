...
function buildCacheKey(prefix: string, params: any) {
...
export async function getTimeSeries(
  ...
  const rows = await db
    ...
    .from(metricsRollup)
    .where(
      sql`${metricsRollup.metric} = ${metric}`,
      sql`${metricsRollup.organizationId} = ${orgId}`,
      sql`${metricsRollup.period} = ${period}`,
      sql`${metricsRollup.tsBucket} BETWEEN ${from} AND ${to}`
    )
