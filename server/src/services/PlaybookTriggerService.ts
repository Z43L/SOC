...
export class PlaybookTriggerService {
  ...
  private async handleIncidentUpdated(event: any): Promise<void> {
  ...
  private async findAndTriggerPlaybooks(triggerType: string, event: any): Promise<void> {
  ...
  private async evaluatePlaybookTrigger(playbook: any, event: any): Promise<boolean> {
    try {
  ...
  private evaluateFilterConditions(filter: Record<string, any>, eventData: Record<string, any>): boolean {
  ...
  private evaluateWhereClause(whereClause: string, eventData: Record<string, any>): boolean {
    try {
  ...
  private evaluateTriggerCondition(condition: any, eventData: Record<string, any>): boolean {
    try {
  ...
  private evaluateEquality(left: string, right: string): boolean {
    try {
  ...
  private evaluateGreaterEqual(left: string, right: string): boolean {
    try {
  ...
  private evaluateInClause(left: string, right: string): boolean {
    try {
      const leftVal = this.parseValue(left);
  ...
  async testTrigger(playbookId: number, sampleEventData: any): Promise<boolean> {
    try {
