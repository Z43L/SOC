...
export type SoarPermission = typeof SOAR_PERMISSIONS[keyof typeof SOAR_PERMISSIONS];

// Permission hierarchy (higher permissions include lower ones)
const PERMISSION_HIERARCHY: Record<SoarPermission, SoarPermission[]> = {
...
export class SoarRbacService {
  ...
  async hasPermission(
    ...
    try {
      ...
      for (const userPerm of userPermissions) {
        const includedPermissions = PERMISSION_HIERARCHY[userPerm as SoarPermission];
  ...
  async getUserPermissions(userId: string, organizationId: number): Promise<string[]> {
    try {
      ...
      const rolePermissions: Record<string, SoarPermission[]> = {
  ...
  async canExecuteAction(
    ...
    try {
      ...
      for (const requirement of actionRequirements) {
        const hasRequirement = await this.hasPermission(userId, organizationId, requirement as SoarPermission);
        if (!hasRequirement) {
          return false;
        }
      }
  ...
  private getActionRequirements(actionName: string): string[] {
  ...
  requirePermission(permission: SoarPermission) {
  ...
  private async getUserWithRole(userId: string, organizationId: number): Promise<any> {
  ...
  private async getPlaybookDetails(playbookId: string): Promise<any> {
  ...
  private extractActionsFromPlaybook(definition: any): string[] {
  ...
  async getPlaybookPermissionRequirements(playbookId: string): Promise<string[]> {
    try {
      const playbook = await this.getPlaybookDetails(playbookId);
      if (!playbook) return [];

      const actions = this.extractActionsFromPlaybook(playbook.definition);
      const requirements = new Set<string>([SOAR_PERMISSIONS.EXECUTE]);
