 ...
 */
export async function generateAlertInsight(alert) {
    try {
 ...
 */
export async function correlateAlerts(alerts) {
    if (alerts.length === 0)
        return null;
    try {
        const openai = ensureOpenAIClient();
 ...
 */
export async function analyzeThreatIntel(intel) {
    try {
        const openai = ensureOpenAIClient();
        const prompt = `
 ...
 */
export async function generateSecurityRecommendations(recentAlerts, recentIncidents) {
    try {
        const openai = ensureOpenAIClient();
