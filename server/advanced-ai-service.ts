...
interface AIModel {
  ...
  multimodal?: boolean; // Si el modelo soporta entradas multimodales (imágenes, audio, etc.)
}
 ...
 */
export function initializeGoogle(apiKey: string): boolean {
  try {
...
function selectModelForAnalysis(
  ...
  try {
    ...
    const selectedModel = AVAILABLE_MODELS.find(model => 
      (provider === ProviderType.OPENAI && model.type === AIModelType.OPENAI && model.name === modelId) ||
    ...
    const providerModel = AVAILABLE_MODELS.find(model => 
      (provider === ProviderType.OPENAI && model.type === AIModelType.OPENAI) ||
...
export async function generateAlertInsight(
  ...
  try {
    ...
    try {
      ...
      const { data, metrics } = await parseOrRetry(
        (options) => orchestrator.generateCompletion(options),
        completionOptions,
      ...
      console.warn("Error using LLM orchestrator for alert insight, falling back to legacy method:", 
