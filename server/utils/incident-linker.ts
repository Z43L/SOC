...
import { Alert, InsertIncident } from '@shared/schema';
 ...
 */
export async function createIncidentIfNeededAndLinkAlert(alert: Alert): Promise<void> {
  try {
    ...
    const relatedIncident = existingIncidents.find(incident => 
    ...
    if (relatedIncident) {
      ...
      const currentAlerts = (relatedIncident.relatedAlerts as number[]) || [];
