...
import { InsertThreatIntel, ThreatFeed } from '@shared/schema';
 ...
 */
export async function fetchOTXData(): Promise<APIResponse> {
  try {
    // Buscar la configuración del feed de OTX en la base de datos
    const otxFeeds = await storage.listThreatFeeds();
    const otxFeed = otxFeeds.find(feed => 
      feed.type.toLowerCase() === 'otx' && 
      feed.isActive && 
      feed.apiKey
    );
    ...
    const threatIntelItems: InsertThreatIntel[] = responseData.results.map((pulse: any) => ({
    ...
    const savedItems = await Promise.all(
      threatIntelItems.map(item => storage.createThreatIntel(item))
    );
 ...
 */
export async function fetchCISAVulnerabilities(): Promise<APIResponse> {
  try {
    ...
    const threatIntelItems: InsertThreatIntel[] = vulnerabilities.map((vuln: any) => ({
      type: 'vulnerability',
 ...
 */
export async function fetchThreatFoxData(): Promise<APIResponse> {
  try {
    ...
    const threatIntelItems: InsertThreatIntel[] = entries.map((entry: any) => ({
...
function determineSeverity(pulse: any): string {
  ...
  if (pulse.TLP === 'AMBER') return 'high';
 ...
 */
function determineSeverityFromThreatFox(entry: any): string {
  ...
  if (confidence > 60 || 
      tags.some((tag: string) => tag.includes('botnet') || tag.includes('banker'))) {
    return 'high';
  }
 ...
 */
function extractIOCs(pulse: any): any {
  const result: any = {};
  ...
  if (pulse.indicators) {
    ...
    if (ips.length > 0) result.ips = ips;
    if (domains.length > 0) result.domains = domains;
 ...
 */
export async function fetchMISPData(): Promise<APIResponse> {
  try {
    // Buscar la configuración del feed de MISP en la base de datos
    const mispFeeds = await storage.listThreatFeeds();
    const mispFeed = mispFeeds.find(feed => 
      feed.type.toLowerCase() === 'misp' && 
      feed.isActive && 
      feed.apiKey
    );
    ...
    if (mispFeed?.apiKey) {
      ...
      if (mispFeed.configuration && 
        ...
        mispUrl = mispFeed.configuration.url as string;
      }
    } else if (apiKey) {
    ...
    const threatIntelItems: InsertThreatIntel[] = responseData.response.map((event: any) => {
 ...
 */
export async function fetchAbuseIPDBData(): Promise<APIResponse> {
  try {
    // Buscar la configuración del feed de AbuseIPDB en la base de datos
    const abuseipdbFeeds = await storage.listThreatFeeds();
    const abuseipdbFeed = abuseipdbFeeds.find(feed => 
      feed.type.toLowerCase() === 'abuseipdb' && 
      feed.isActive && 
      feed.apiKey
    );
    ...
    const threatIntelItems: InsertThreatIntel[] = [];
 ...
 */
export async function importAllFeeds(): Promise<APIResponse> {
  try {
