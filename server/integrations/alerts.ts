 ...
 */
export async function fetchVirusTotalAlerts(): Promise<APIResponse> {
  try {
    ...
    if (!response.ok) {
      ...
      const threatData = await publicFeedResponse.json() as any;
      ...
      const savedAlerts = await Promise.all(
        alerts.map(alert => storage.createAlert(alert))
      );
    ...
    const vtData = await response.json() as any;
    const alerts: InsertAlert[] = [];
 ...
 */
export async function fetchSuricataAlerts(): Promise<APIResponse> {
  try {
 ...
 */
export async function fetchMISPAlerts(): Promise<APIResponse> {
  try {
        ...
        if (threatFoxResponse.ok) {
          ...
          if (threatFoxData.data && Array.isArray(threatFoxData.data)) {
            ...
            topFamilies.forEach(family => {
              const severity = 
                family.tags.includes("apt") || family.tags.includes("ransomware") ? "critical" :
 ...
 */
export async function importAllAlerts(): Promise<APIResponse> {
  try {
    ...
    const totalAlerts = 
      (vtResult.success ? (vtResult.data?.length || 0) : 0) +
