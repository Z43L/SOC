...
interface BehaviorPattern {
  ...
  technique: CorrelationTechnique;
  rules: string[];
  metadata: Record<string, any>;
}

// Opciones para técnicas de correlación
interface CorrelationOptions {
  timeWindowHours: number;
  confidenceThreshold: number;
  techniques: CorrelationTechnique[];
  includeAiInsights: boolean;
  maxEntitiesPerPattern: number;
...
class TemporalCorrelator {
   ...
   */
  async findPatterns(alertsInput: Alert[]): Promise<BehaviorPattern[]> {
        ...
        patterns.push({
          ...
          metadata: {
            ...
            timespan: this.calculateTimespan(
              new Date(sequence.alerts[0].timestamp || Date.now()),
              new Date(sequence.alerts[sequence.alerts.length - 1].timestamp || Date.now())
            )
          }
        });
      }
   ...
   */
  private extractEventSequences(alerts: Alert[]): Array<{
    events: string[];
    confidence: number;
    alerts: Alert[];
  }> {
   ...
   */
  private normalizeEventType(alert: Alert): string {
    ...
    if (['firewall', 'waf', 'ids', 'ips', 'edr', 'xdr', 'ndr', 'dlp'].some(src => 
        alert.source.toLowerCase().includes(src))) {
   ...
   */
  private calculateTimeFactor(alerts: Alert[]): number {
   ...
   */
  private calculateSeverityFactor(alerts: Alert[]): number {
    const severityMap = {
      'critical': 1.0,
      'high': 0.75,
      'medium': 0.5,
   ...
   */
  private calculateSourceFactor(alerts: Alert[]): number {
...
class GraphCorrelator {
   ...
   */
  async findPatterns(alerts: Alert[], threatIntel?: ThreatIntel[]): Promise<BehaviorPattern[]> {
    const patterns: BehaviorPattern[] = [];
    ...
    for (const community of communities) {
      if (community.nodes.length >= 2 && community.density > 0.5) {
        ...
        const communityAlerts = alerts.filter(alert => 
          community.nodes.some(node => 
            node.type === 'alert' && node.id === alert.id
          )
        );
        
        // Extraer IoCs principales
        const primaryIoCs: string[] = [];
   ...
   */
  private async buildEntityGraph(alerts: Alert[], threatIntel?: ThreatIntel[]): Promise<{
    nodes: Array<{
      id: number;
      type: string;
      data: any;
    ...
    const ipToNodes = new Map<string, Array<{ type: string; id: number }>>();
    const domainToNodes = new Map<string, Array<{ type: string; id: number }>>();
    const hashToNodes = new Map<string, Array<{ type: string; id: number }>>();
    ...
    for (const node of graph.nodes) {
      if (node.type === 'intel') {
        const intel = node.data as ThreatIntel;
    ...
    const alertsBySource = new Map<string, Alert[]>();
    for (const alert of alerts) {
        ...
        for (let i = 0; i < sourceAlerts.length; i++) {
          for (let j = i + 1; j < sourceAlerts.length; j++) {
            ...
            if (timeDiff <= this.options.timeWindowHours * 60 * 60 * 1000) {
              addEdge(
                { type: 'alert', id: sourceAlerts[i].id },
                { type: 'alert', id: sourceAlerts[j].id },
                'same_source',
                0.6, // Peso para misma fuente
                { source }
              );
            }
          }
   ...
   */
  private detectCommunities(graph: {
    nodes: Array<{
      id: number;
      type: string;
      data: any;
    ...
    for (const node of graph.nodes) {
      ...
      if (!adjacencyMap.has(nodeKey)) {
        adjacencyMap.set(nodeKey, new Set<string>());
      }
    }
    ...
    for (let i = 0; i < communities.length; i++) {
      ...
      const communityEdges = graph.edges.filter(edge => 
        community.has(`${edge.source.type}:${edge.source.id}`) && 
      ...
      if (centralNodes.length > 0) {
        ...
        if (node) {
          if (node.type === 'alert') {
            const alert = node.data as Alert;
...
export class AdvancedCorrelationAlgorithms {
  private defaultOptions: CorrelationOptions = {
    timeWindowHours: 24,
  ...
  private temporalCorrelator: TemporalCorrelator;
  private graphCorrelator: GraphCorrelator;
   ...
   */
  async correlate(alerts: Alert[], threatIntel?: ThreatIntel[]): Promise<{
    patterns: BehaviorPattern[];
    incidentSuggestions: Partial<Incident>[];
  }> {
   ...
   */
  private patternToIncident(pattern: BehaviorPattern, alerts: Alert[]): Partial<Incident> {
    ...
    let severity: SeverityTypes = 'medium';
   ...
   */
  async analyzeAndSuggestIncidents(timeWindowHours: number = 24): Promise<Incident[]> {
    try {
      ...
      for (const suggestion of result.incidentSuggestions) {
        try {
          ...
          const relatedAlerts = recentAlerts.filter(alert => 
            suggestion.relatedAlerts?.includes(alert.id)
          );
          ...
          if (relatedAlerts.length > 0) {
            ...
            let finalIncident: Partial<Incident>;
            ...
            if (aiResult && aiResult.confidence > suggestion.aiAnalysis?.confidence!) {
              ...
              finalIncident = {
                ...
                severity: aiResult.severity as SeverityTypes,
                status: 'new',
            ...
            // Crear el incidente
            const incident = await storage.createIncident(finalIncident as any);
            if (incident) {
