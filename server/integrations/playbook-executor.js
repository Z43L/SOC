...
export var PlaybookStepType;
(function (PlaybookStepType) {
    // Pasos de integración con sistemas EDR
    PlaybookStepType["EDR_ISOLATE_HOST"] = "edr_isolate_host";
    PlaybookStepType["EDR_UNISOLATE_HOST"] = "edr_unisolate_host";
    PlaybookStepType["EDR_SCAN_HOST"] = "edr_scan_host";
    PlaybookStepType["EDR_GET_PROCESS_LIST"] = "edr_get_process_list";
    PlaybookStepType["EDR_KILL_PROCESS"] = "edr_kill_process";
    // Pasos de integración con firewalls
    PlaybookStepType["FIREWALL_BLOCK_IP"] = "firewall_block_ip";
    PlaybookStepType["FIREWALL_UNBLOCK_IP"] = "firewall_unblock_ip";
    PlaybookStepType["FIREWALL_BLOCK_DOMAIN"] = "firewall_block_domain";
    PlaybookStepType["FIREWALL_UNBLOCK_DOMAIN"] = "firewall_unblock_domain";
    // Pasos de integración con sistemas de identidad
    PlaybookStepType["IDENTITY_DISABLE_USER"] = "identity_disable_user";
    PlaybookStepType["IDENTITY_ENABLE_USER"] = "identity_enable_user";
    PlaybookStepType["IDENTITY_RESET_PASSWORD"] = "identity_reset_password";
    PlaybookStepType["IDENTITY_ADD_TO_GROUP"] = "identity_add_to_group";
    PlaybookStepType["IDENTITY_REMOVE_FROM_GROUP"] = "identity_remove_from_group";
    // Pasos de notificación
    PlaybookStepType["NOTIFY_EMAIL"] = "notify_email";
    PlaybookStepType["NOTIFY_SLACK"] = "notify_slack";
    PlaybookStepType["NOTIFY_SMS"] = "notify_sms";
    // Pasos de enriquecimiento y análisis
    PlaybookStepType["ENRICH_IOC"] = "enrich_ioc";
    PlaybookStepType["AI_ANALYZE_ALERT"] = "ai_analyze_alert";
    PlaybookStepType["LOOKUP_THREAT_INTEL"] = "lookup_threat_intel";
    // Flujo de control
    PlaybookStepType["CONDITION"] = "condition";
    PlaybookStepType["WAIT"] = "wait";
    PlaybookStepType["PARALLEL"] = "parallel";
    PlaybookStepType["CALL_API"] = "call_api";
})(PlaybookStepType || (PlaybookStepType = {}));
...
export class PlaybookExecutor {
    ...
    findStartingSteps() {
        ...
        return this.playbook.steps.filter(step => !nextStepIds.has(step.id) ||
    ...
    async executeStep(step) {
        try {
            ...
            for (const nextStepId of nextSteps) {
                const nextStep = this.playbook.steps.find(s => s.id === nextStepId);
                if (nextStep) {
    ...
    async finishExecution(success, errorMessage) {
    ...
    async executeEdrIsolateHost(step) {
        try {
    ...
    async executeEdrUnisolateHost(step) {
        try {
            const { host, edrSystem } = step.config;
    ...
    async executeEdrGetProcessList(step) {
        try {
            const { host, edrSystem } = step.config;
    ...
    async executeEdrKillProcess(step) {
        try {
    ...
    // Firewall
    async executeFirewallBlockIp(step) {
        try {
    ...
    async executeFirewallUnblockIp(step) {
        try {
    ...
    async executeFirewallBlockDomain(step) {
        try {
    ...
    async executeFirewallUnblockDomain(step) {
        try {
    ...
    // Identity Management
    async executeIdentityDisableUser(step) {
        try {
    ...
    async executeIdentityEnableUser(step) {
        try {
            const { username, identitySystem, reason } = step.config;
    ...
    async executeIdentityResetPassword(step) {
        try {
    ...
    async executeIdentityAddToGroup(step) {
        try {
    ...
    async executeIdentityRemoveFromGroup(step) {
        try {
            const { username, groupName, identitySystem } = step.config;
    ...
    async executeAiAnalyzeAlert(step) {
        try {
    ...
    async executeLookupThreatIntel(step) {
        try {
    ...
    async executeParallel(step) {
        try {
            ...
            const stepsToExecute = steps
                .map(stepId => this.playbook.steps.find(s => s.id === stepId))
                .filter(Boolean);
            // Ejecutar todos los pasos en paralelo
            const results = await Promise.all(stepsToExecute.map(step => this.executeStep(step)));
    ...
    async callExternalApi({ url, method, headers, body, timeout = 10000 }) {
        try {
...
export const playbookExecutorService = {
     ...
     */
    async executePlaybook(playbookId, triggeredBy, triggerEntityId, triggerSource) {
        try {
