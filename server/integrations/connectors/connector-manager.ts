...
import { IDataConnector, ConnectorConfig, ConnectorType, ConnectorStatus, WebhookConfig } from './interfaces';
...
import { connectors, connectorLogs, eventsRaw, insertsRaw, InsertConnector } from '@shared/schema';
...
export class ConnectorManager extends EventEmitter {
  ...
  async shutdown(): Promise<void> {
    ...
    const stopPromises = Array.from(this.connectors.values()).map(connector => 
      connector.stop().catch(err => 
        log(`Error deteniendo conector ${connector.id}: ${err}`, 'connector-manager')
      )
    );
   ...
   */
  async createConnector(config: ConnectorConfig): Promise<string> {
    try {
      const insertData: InsertConnector = {
        name: config.name,
        type: config.type,
        vendor: 'Generic',
        status: 'active',
  ...
  async updateConnector(id: string, updates: Partial<ConnectorConfig>): Promise<void> {
    try {
      ...
      const updateData: Partial<InsertConnector> = {
   ...
   */
  getConnector(id: string): IDataConnector | undefined {
   ...
   */
  getAllConnectors(): IDataConnector[] {
   ...
   */
  private createConnectorInstance(config: ConnectorConfig): IDataConnector {
    switch (config.type) {
      case 'syslog':
        return new SyslogConnector(config as any);
      
      case 'api':
        return new ApiPollingConnector(config);
      
      case 'webhook':
        return new WebhookConnector(config as WebhookConfig);
      
      case 'file':
        return new FileConnector(config as any);
   ...
   */
  private async loadActiveConnectors(): Promise<ConnectorRecord[]> {
   ...
   */
  private async loadConnector(id: number): Promise<ConnectorRecord | null> {
    const result = await db.select().from(connectors)
   ...
   */
  private async createAndStartConnector(data: ConnectorRecord): Promise<void> {
    try {
   ...
   */
  private setupConnectorListeners(connector: IDataConnector): void {
    connector.on('event', async (event) => {
      this.emit('event', event);
      // Broadcast raw event over WebSocket
      try { getIo().emit('connector-event', event); } catch {}
      // Persist event log
      try {
    ...
    connector.on('error', async (error) => {
      ...
      try { getIo().emit('connector-status', { id: connector.id, status: 'error' }); } catch {}
      // Persist error log
      try {
        await db.insert(connectorLogs)
    ...
    connector.on('status-change', async (status) => {
      ...
      try { getIo().emit('connector-status', { id: connector.id, status }); } catch {}
    ...
    connector.on('metrics-update', async (metrics) => {
      ...
      try { getIo().emit('connector-metrics', { id: connector.id, metrics }); } catch {}
   ...
   */
  private async updateConnectorStatus(id: string, status: ConnectorStatus, message?: string): Promise<void> {
    try {
      const connectorId = parseInt(id);
   ...
   */
  private setupDatabaseNotifications(): void {
    ...
    pool.connect().then(client => {
      ...
      client.query('LISTEN connectors_changed').catch(err =>
        log(`Failed to LISTEN on connectors_changed: ${err}`, 'connector-manager')
      );
    }).catch(err =>
      log(`Failed to setup DB notifications: ${err}`, 'connector-manager')
    );
  }
   ...
   */
  private async handleDbNotification(id: string): Promise<void> {
    try {
   ...
   */
  public async testConnector(id: string): Promise<{ success: boolean; message: string }> {
   ...
   */
  public async pauseConnector(id: string): Promise<void> {
    const connector = this.connectors.get(id);
    if (!connector) {
      throw new Error(`Conector ${id} no encontrado`);
    }
   ...
   */
  public async resumeConnector(id: string): Promise<void> {
   ...
   */
  public async toggleConnector(id: string, status: ConnectorStatus): Promise<void> {
    if (status === 'paused') {
