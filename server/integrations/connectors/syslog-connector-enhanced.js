 ...
 */
export class SyslogConnector extends EventEmitter {
    id;
    organizationId;
    name;
    type = 'syslog';
    ...
    async updateConfig(config) {
        const needsRestart = (config.protocol !== undefined && config.protocol !== this._config.protocol ||
     ...
     */
    parseSyslogMessage(message, sourceIp) {
     ...
     */
    createEventFromSyslogMessage(parsedMessage) {
    ...
    shouldProcessMessage(message) {
        ...
        if (config.filtering.sources && config.filtering.sources.length > 0) {
            if (!config.filtering.sources.some(source => message.hostname === source || message.sourceIp === source)) {
                return false;
            }
        }
        // Check include patterns
        if (config.filtering.include && config.filtering.include.length > 0) {
            if (!config.filtering.include.some(pattern => message.rawMessage.includes(pattern) || message.message.includes(pattern))) {
                return false;
            }
        }
        // Check exclude patterns
        if (config.filtering.exclude && config.filtering.exclude.length > 0) {
            if (config.filtering.exclude.some(pattern => message.rawMessage.includes(pattern) || message.message.includes(pattern))) {
                return false;
            }
        }
        return true;
