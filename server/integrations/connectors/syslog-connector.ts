...
import { SyslogConfig, HealthCheckResult } from './interfaces';
...
export class SyslogConnector extends BaseConnector {
  private server?: dgram.Socket | net.Server;
   ...
   * Inicia servidor UDP
   */
  private async startUdpServer(port: number, bindAddress: string): Promise<void> {
    return new Promise((resolve, reject) => {
   ...
   * Inicia servidor TCP
   */
  private async startTcpServer(port: number, bindAddress: string): Promise<void> {
    return new Promise((resolve, reject) => {
   ...
   */
  private processSyslogMessage(rawMessage: string, sourceIp: string): void {
    try {
   ...
   */
  private mapSyslogSeverity(severity: number): 'info' | 'warn' | 'error' | 'critical' {
    switch (severity) {
      case 0: case 1: case 2: return 'critical';
      case 3: case 4: return 'error';
      case 5: return 'warn';
      default: return 'info';
    }
  }
