...
export class AgentConnector extends BaseConnector {
  ...
  private async processAgentEvents(): Promise<void> {
    ...
    for (const event of eventsToProcess) {
      try {
        // Solo crear alertas para ciertos tipos de eventos
        if (this.shouldCreateAlert(event)) {
          const alert = this.createAlertFromAgentEvent(event);
          if (alert) {
        ...
        log(`Error procesando evento de agente: ${error instanceof Error ? error.message : 'Error desconocido'}`, 'connector');
      }
    }
    
    log(`Creadas ${alerts.length} alertas de ${eventsToProcess.length} eventos de agentes`, 'connector');
  }
  ...
  private async checkInactiveAgents(): Promise<void> {
    ...
    for (const [agentId, agent] of Object.entries(this.config.agents || {})) {
      ...
      // Marcar como inactivo si supera el timeout
      if (timeDiff > timeoutMs && agent.status === 'active') {
        log(`Agente ${agent.hostname} (${agentId}) marcado como inactivo por falta de comunicación`, 'connector');
