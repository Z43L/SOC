...
export class DataProcessor {
     ...
     */
    async processData(rawData, dataSource) {
     ...
     */
    async processRecord(record, dataSource, result) {
     ...
     */
    async normalizeThreatIntel(data) {
        try {
            const normalized = {
                organizationId: this.connector.organizationId,
     ...
     */
    async processWithAI(data, dataSource) {
        if (!aiParser) {
            return { alerts: [] };
        }
        try {
     ...
     */
    inferDataType(data) {
        if (data.alert || data.event_type === 'alert')
            return 'alert';
        if (data.indicators || data.iocs || data.threat_level)
            return 'threatIntel';
        if (data.severity || data.priority)
            return 'alert';
        return 'unknown';
    }
    /**
     ...
     */
    extractMISPAttributes(attributes) {
     ...
     */
    async normalizeGenericData(data) {
        try {
            return {
