...
/**
 * Conector para OTX AlienVault
 * Permite obtener información de inteligencia de amenazas desde la API de OTX AlienVault
 */
export class OTXConnector extends APIConnector {
  constructor(connector: any) {
    super(connector);
    
    // Asegurar configuración predeterminada para OTX
    const config = this.connector.configuration as APIConnectorConfig;
    
    // Establecer valores predeterminados si no están configurados
    if (!config.baseUrl) {
      config.baseUrl = 'https://otx.alienvault.com/api/v1';
    }
    
    if (!config.apiKeyHeader) {
      config.apiKeyHeader = 'X-OTX-API-KEY';
    }
    ...
    if (!config.endpoints) {
      config.endpoints = {
        ...
        indicators: {
          ...
          params: {
            limit: '20',
            types: 'domain,hostname,IPv4,url,FileHash-MD5,FileHash-SHA1,FileHash-SHA256'
          }
        }
      };
    }
    ...
    // Utilizar API key del environment si no está configurada
    if (!config.apiKey && process.env.OTX_API_KEY) {
      config.apiKey = process.env.OTX_API_KEY;
      log('Usando API key de OTX AlienVault desde variables de entorno', 'connector');
    }
    
    // Configurar paginación
    config.paginate = true;
    config.paginationParamName = 'page';
    config.paginationTokenPath = 'next';
  ...
  /**
   * Procesa datos recibidos de OTX y los convierte al formato de inteligencia
   * @override
   */
  protected async processThreatIntel(data: any[]): Promise<InsertThreatIntel[]> {
    const intel: InsertThreatIntel[] = [];
    ...
    for (const pulse of data) {
      try {
        ...
        // Solo agregar si hay IOCs válidos
        if (Object.keys(iocs).length === 0) {
          continue;
        }
        
        const threatIntel: InsertThreatIntel = {
          type: (pulse.tags && pulse.tags[0]) || 'malware',
          title: pulse.name || 'OTX Intel',
          description: pulse.description || `Intel from ${this.connector.name}`,
        ...
        if (pulse.created) {
          threatIntel.createdAt = new Date(pulse.created);
        }
        
        intel.push(threatIntel);
      } catch (itemError) {
        log(`Error procesando item de OTX: ${itemError instanceof Error ? itemError.message : 'Error desconocido'}`, 'connector');
      }
    }
  ...
  /**
   * Mapea niveles TLP a severidad del sistema
   */
  private mapSeverity(tlp: string | undefined): string {
