 ...
 */
export async function initializeConnectors(app: express.Application): Promise<void> {
  try {
 ...
 */
function setupConnectorRoutes(app: express.Application): void {
  ...
  router.get('/', async (req, res) => {
    try {
      ...
      res.status(500).json({ 
        success: false, 
        message: `Error obteniendo conectores: ${error instanceof Error ? error.message : 'Error desconocido'}` 
      });
    }
  });
