...
(function (DataFormat) {
    ...
    DataFormat["TAXII"] = "taxii";
    DataFormat["CEF"] = "cef";
    DataFormat["LEEF"] = "leef";
    DataFormat["ECS"] = "ecs";
    DataFormat["WINDOWS_EVENT"] = "wef";
    DataFormat["SYSLOG"] = "syslog";
    DataFormat["JSON"] = "json";
    DataFormat["CSV"] = "csv";
    DataFormat["XML"] = "xml";
    DataFormat["PLAINTEXT"] = "text";
    DataFormat["UNKNOWN"] = "unknown"; // Desconocido
...
export class StructuredDataParser {
     ...
     */
    extractStixSeverity(stixObj) {
        // Verificar etiquetas para indicaciones de severidad
        if (stixObj.labels && Array.isArray(stixObj.labels)) {
            if (stixObj.labels.some((label) => label.toLowerCase().includes('critical') ||
                label.toLowerCase().includes('severe'))) {
                return 'critical';
            }
            if (stixObj.labels.some((label) => label.toLowerCase().includes('high'))) {
                return 'high';
            }
            if (stixObj.labels.some((label) => label.toLowerCase().includes('medium') ||
                label.toLowerCase().includes('moderate'))) {
                return 'medium';
            }
            if (stixObj.labels.some((label) => label.toLowerCase().includes('low'))) {
                return 'low';
            }
        }
        // Extraer de nivel de confianza si existe
        if (typeof stixObj.confidence === 'number') {
            if (stixObj.confidence >= 85)
                return 'high';
            if (stixObj.confidence >= 60)
                return 'medium';
            return 'low';
        }
        // Valor predeterminado
     ...
     */
    extractIocsFromStixPattern(pattern) {
        const iocs = {
            ips: [],
            domains: [],
            hashes: [],
     ...
     */
    extractIocsFromStixObservable(observable) {
        const iocs = {
            ips: [],
            domains: [],
            hashes: [],
    ...
    mapCefSeverity(severity) {
        ...
        if (sevLower.includes('critical'))
            return 'critical';
        if (sevLower.includes('high'))
            return 'high';
        if (sevLower.includes('medium'))
            return 'medium';
        if (sevLower.includes('low'))
            return 'low';
        return 'medium';
    }
    /**
    ...
    async parseWindowsEvent(data) {
        ...
        try {
            ...
            let severity = 'medium';
            if (level === 1)
                severity = 'critical'; // Error crítico
            else if (level === 2)
                severity = 'high'; // Error
            else if (level === 3)
                severity = 'high'; // Advertencia
            else if (level === 4)
                severity = 'medium'; // Información
            else if (level === 5)
                severity = 'low'; // Detalle
            ...
            if (eventData.Hashes) {
                ...
                for (const hash of hashes) {
                    if (hash.includes('MD5=')) {
                        ...
                        if (!parsedData.iocs)
                            parsedData.iocs = {};
                        if (!parsedData.iocs.hashes)
                            parsedData.iocs.hashes = [];
     ...
     */
    extractField(obj, possibleNames) {
    ...
    normalizeSeverity(severity) {
        ...
        if (!isNaN(sevNum)) {
            // Diferentes escalas
            if (sevNum >= 9 && sevNum <= 10)
                return 'critical';
            if (sevNum >= 7 && sevNum <= 8)
                return 'high';
            if (sevNum >= 4 && sevNum <= 6)
                return 'medium';
            if (sevNum >= 0 && sevNum <= 3)
                return 'low';
    ...
    extractIoCsFromText(text) {
        ...
        iocs.domains = [...new Set(domains.filter(d => !d.match(/^\d+\.\d+$/) &&
                !d.match(/^\d+\.\d+\.\d+$/) &&
                !d.includes('@')))];
        // Extraer URLs
