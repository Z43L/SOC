...
(function (DataFormat) {
    ...
    DataFormat["XML"] = "xml";
    DataFormat["SYSLOG"] = "syslog";
    DataFormat["CEF"] = "cef";
    DataFormat["LEEF"] = "leef";
    DataFormat["CSV"] = "csv";
    DataFormat["PLAINTEXT"] = "plaintext";
    DataFormat["STIX"] = "stix";
...
(function (DataType) {
    ...
    DataType["LOG"] = "log";
    DataType["THREAT_INTEL"] = "threat_intel";
    DataType["METRIC"] = "metric";
    DataType["NETWORK_TRAFFIC"] = "network_traffic";
...
export class AIParserService {
     ...
     */
    async parseToAlert(rawData, connector) {
     ...
     */
    async parseToThreatIntel(rawData, connector) {
        // Resultado por defecto
        const result = {
            success: false,
            confidence: 0,
     ...
     */
    async parseLogs(rawLogs, connector) {
     ...
     */
    determineDataSource(rawData, format) {
        const dataStr = typeof rawData === 'object' ? JSON.stringify(rawData) : String(rawData);
     ...
     */
    async parseWithConventionalParsers(rawData, format, source, dataType) {
        const result = {
            success: false,
            data: undefined,
            confidence: 0,
     ...
     * Parsea datos JSON
     */
    parseJsonData(rawData, source, dataType) {
        const result = {
            success: false,
            data: undefined,
            confidence: 0,
     ...
     */
    parseXmlData(rawData, source, dataType) {
     ...
     */
    parseCefData(rawData, source, dataType) {
        const result = {
            success: false,
            data: undefined,
            confidence: 0,
     ...
     * Parsea datos Syslog
     */
    parseSyslogData(rawData, source, dataType) {
        const result = {
            success: false,
            data: undefined,
            confidence: 0,
     ...
     */
    parseStixData(rawData, source, dataType) {
        const result = {
            success: false,
            data: undefined,
            confidence: 0,
        ...
        try {
            ...
            if (data.object_marking_refs) {
                const tlpMarkings = Array.isArray(data.object_marking_refs) ?
                    data.object_marking_refs.filter((ref) => ref.includes('tlp')) : [];
     ...
     */
    parseCsvData(rawData, source, dataType) {
        const result = {
            success: false,
            data: undefined,
            confidence: 0,
     ...
     */
    parseLeefData(rawData, source, dataType) {
        const result = {
            success: false,
            data: undefined,
            confidence: 0,
     ...
     */
    async parseWithAI(rawData, dataType, format, source) {
        const result = {
            success: false,
            data: undefined,
            confidence: 0,
     ...
     */
    normalizeAlert(data, extractedIocs, source) {
     ...
     */
    normalizeThreatIntel(data, extractedIocs, source) {
        // Asegurar que tenemos todos los campos necesarios
        const intel = {
    ...
    validateThreatIntel(intel) {
        ...
        const hasIocs = intel.iocs && ((Array.isArray(intel.iocs.ips) && intel.iocs.ips.length > 0) ||
    ...
    extractIoCs(data) {
        ...
        result.domains = result.domains.filter(domain => !domain.includes('@') &&
     ...
     */
    generateCacheKey(rawData, dataType, source) {
