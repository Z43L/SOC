 ...
 */
function extractIOCsFromAlert(alert) {
    const result = {
        ips: [],
        domains: [],
        hashes: [],
    ...
    if (alert.metadata) {
        ...
        if (md5s)
            result.hashes.push(...md5s);
        if (sha1s)
            result.hashes.push(...sha1s);
        if (sha256s)
            result.hashes.push(...sha256s);
...
async function findRelatedThreatIntel(iocs) {
    ...
    for (const intel of allIntel) {
        ...
        if (!matches && iocs.domains.length > 0 && intelIOCs && 'domains' in intelIOCs) {
            const intelDomains = intelIOCs.domains;
        ...
        if (!matches && iocs.hashes.length > 0 && intelIOCs && 'hashes' in intelIOCs) {
            const intelHashes = intelIOCs.hashes;
        ...
        if (!matches && iocs.urls.length > 0 && intelIOCs && 'urls' in intelIOCs) {
            const intelURLs = intelIOCs.urls;
 ...
 */
export async function enrichAlertWithThreatIntel(alert) {
    try {
        ...
        if (typeof metadata === 'object') {
            metadata.threatIntel = metadata.threatIntel || {
                matchedEntries: [],
                iocs: iocs,
                enrichmentTime: new Date().toISOString()
            };
            const tiMetadata = metadata.threatIntel;
            ...
            const criticalThreats = matchedIntel.filter(intel => intel.severity === 'critical' ||
 ...
 */
export async function processNewAlertWithEnrichment(alert) {
