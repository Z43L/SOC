...
import { ThreatIntel, InsertThreatIntel, SeverityTypes } from '@shared/schema';
import { log } from '../vite';
import { storage } from '../storage';
import { aiParser } from './ai-parser-service';
import { aiQueue } from './ai-processing-queue';

// Interfaz para la configuración del conector TAXII
interface TaxiiConfig {
  apiRoot: string;           // URL raíz de la API TAXII
  collectionId: string;      // ID de la colección a consultar
  username?: string;         // Credenciales (opcional)
  password?: string;         // Credenciales (opcional)
  apiKey?: string;           // API key alternativa (opcional)
  version: '2.0' | '2.1';    // Versión de TAXII
  lastFetchTimestamp?: Date; // Última fecha de obtención
  maxItems?: number;         // Máximo de elementos a obtener
  userLabels?: string[];     // Etiquetas definidas por el usuario
}

// Interfaz para objetos STIX
...
export class TaxiiConnector {
   ...
   */
  constructor(config: TaxiiConfig, connectorId?: number) {
    this.config = config;
   ...
   */
  async getServerInformation(): Promise<any> {
    try {
   ...
   */
  private stixToThreatIntel(stixObject: StixObject): InsertThreatIntel {
   ...
   */
  private getSourceName(stixObject: StixObject): string {
   ...
   */
  private mapStixTypeToIntelType(stixType: string): string {
    switch (stixType) {
      case 'indicator':
        return 'indicator';
      case 'malware':
   ...
   */
  private extractIoCsFromPattern(pattern: string, iocs: Record<string, string[]>): void {
    // IPv4
   ...
   */
  private extractIoCsFromProperties(stixObject: StixObject, iocs: Record<string, string[]>): void {
    // Extraer IPs
    if (stixObject.x_observable_properties?.ipv4_addrs) {
      stixObject.x_observable_properties.ipv4_addrs.forEach((ip: string) => {
        iocs.ips.push(ip);
      });
    }
    ...
    if (stixObject.x_observable_properties?.domain_names) {
      stixObject.x_observable_properties.domain_names.forEach((domain: string) => {
    ...
    if (stixObject.x_observable_properties?.urls) {
      stixObject.x_observable_properties.urls.forEach((url: string) => {
        iocs.urls.push(url);
      });
    }
  ...
  async importIntelligence(options: {
    ...
    try {
      ...
      const intelItems: InsertThreatIntel[] = [];
      let errors = 0;
...
export function createTaxiiConnector(config: TaxiiConfig, connectorId?: number): TaxiiConnector {
