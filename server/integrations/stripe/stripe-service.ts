...
export class StripeService {
   ...
   */
  async createCheckoutSession(userId: number, email: string, planId: number, isYearly: boolean) {
    try {
      // Obtener el plan
   ...
   */
  async createBillingPortalSession(userId: number) {
    try {
      const user = await storage.getUser(userId);
   ...
   */
  private async getDefaultSubscriptionData(userId: number) {
    try {
