  ...
  private async packageAgent(
    ...
    try {
      ...
      switch (os) {
        ...
        case AgentOS.MACOS:
          outputFileName = `soc-agent-macos-${agentId}.zip`;
          outputFilePath = path.join(this.outputDir, outputFileName);
...
const { WindowsAgent } = require('./agent-core');
...
const AgentClass = require(mainModulePath)['${this.getAgentClassName(os)}'];
...
`;
  ...
  private async createTarArchive(sourceDir: string, outputPath: string): Promise<void> {
    // Cambiar al directorio y empaquetar todo
    await exec(`cd "${sourceDir}" && tar -czf "${outputPath}" .`);
  }
