...
import { db } from '../db';
import { plans, organizations } from '@shared/schema';
...
export class StripeService {
     ...
     */
    static async createOrUpdatePlanPrices(plan) {
        try {
            if (!this.isConfigured()) {
                console.error('Stripe no está configurado');
                return {};
     ...
     */
    static async createSubscription(organizationId, planId, billingCycle) {
        try {
            if (!this.isConfigured()) {
                console.error('Stripe no está configurado');
                return null;
     ...
     */
    static async cancelSubscription(organizationId) {
        try {
            if (!this.isConfigured()) {
                console.error('Stripe no está configurado');
                return false;
     ...
     */
    static async changePlan(organizationId, newPlanId, billingCycle) {
        try {
            if (!this.isConfigured()) {
                console.error('Stripe no está configurado');
                return false;
            ...
            const cycleToUse = billingCycle || organization.billingCycle;
     ...
     */
    static async createCheckoutSession(organizationId, planId, billingCycle, successUrl, cancelUrl) {
        try {
            if (!this.isConfigured()) {
                console.error('Stripe no está configurado');
                return null;
     ...
     */
    static async createCustomerPortalSession(organizationId, returnUrl) {
        try {
            if (!this.isConfigured()) {
                console.error('Stripe no está configurado');
                return null;
