...
export class AICorrelationEngine {
     ...
     */
    async analyzeAlertGroup(alerts) {
     ...
     */
    async findCorrelationPatterns(timeWindowHours = this.thresholds.timeWindowHours) {
        try {
     ...
     */
    async buildCorrelationContext(alerts) {
        ...
        for (const alert of alerts) {
            // IPs de la alerta
            if (alert.sourceIp)
                context.iocs.ips.add(alert.sourceIp);
            if (alert.destinationIp)
                context.iocs.ips.add(alert.destinationIp);
        ...
        context.insights = allInsights.filter(insight => {
            ...
            if (insight.type === 'alert_analysis') {
                ...
                const mentionsAlert = alerts.some(alert => insight.description?.includes(`Alert ID: ${alert.id}`) ||
            ...
            if (Array.isArray(insight.relatedEntities)) {
                for (const entity of insight.relatedEntities) {
                    if (typeof entity === 'string') {
                        ...
                        if (context.iocs.ips.has(entity))
                            return true;
                        if (context.iocs.domains.has(entity))
                            return true;
                        if (context.iocs.hashes.has(entity))
                            return true;
                        if (context.iocs.urls.has(entity))
                            return true;
                    }
                }
            }
    ...
    performRiskAnalysis(context) {
        ...
        if (context.relatedEntities.threatActors.size > 0) {
            const threatActorImpact = Math.min(this.riskFactors.knownThreatActorBonus, context.relatedEntities.threatActors.size * 2);
    ...
    groupRelatedAlerts(alerts) {
        ...
        return Object.fromEntries(Object.entries(groups).filter(([_, groupAlerts]) => groupAlerts.length >= 2));
    }
    /**
     ...
     */
    generateCorrelationTitle(context) {
    ...
    summarizeEvidence(context) {
        ...
        if (context.iocs.ips.size > 0 || context.iocs.domains.size > 0 || context.iocs.hashes.size > 0) {
            const iocCounts = [];
            if (context.iocs.ips.size > 0)
                iocCounts.push(`${context.iocs.ips.size} IPs`);
            if (context.iocs.domains.size > 0)
                iocCounts.push(`${context.iocs.domains.size} dominios`);
            if (context.iocs.hashes.size > 0)
                iocCounts.push(`${context.iocs.hashes.size} hashes`);
     ...
     */
    extractIoCsFromObject(obj, result) {
     ...
     */
    async findRelatedThreatIntel(iocs) {
                ...
                try {
                    ...
                    if (metadata.iocs) {
                        ...
                        if (!matched && Array.isArray(metadata.iocs.domains)) {
                            for (const domain of iocs.domains) {
                                if (metadata.iocs.domains.some((d) => d.toLowerCase() === domain.toLowerCase())) {
                                    matched = true;
                                    break;
                                }
                            }
                        ...
                        if (!matched && Array.isArray(metadata.iocs.hashes)) {
                            for (const hash of iocs.hashes) {
                                if (metadata.iocs.hashes.some((h) => h.toLowerCase() === hash.toLowerCase())) {
                                    matched = true;
                                    break;
                                }
                            }
