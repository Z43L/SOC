...
async function pushConfigurationUpdate(agentId: string, config: any) {
  try {
    ...
    if (pushNotificationConfig.retryPolicy.maxRetries > 0) {
      await new Promise(resolve => 
        setTimeout(resolve, pushNotificationConfig.retryPolicy.initialDelay)
      );
...
async function exponentialBackoff<T>(fn: () => Promise<T>, retries = MAX_RETRIES): Promise<T> {
  try {
    return await fn();
  } catch (error) {
...
export async function processAgentHeartbeat(
  ...
  try {
    ...
    let agentId: number | null = null;
    let currentAgent: any = null; // Use currentAgent to store the agent object
    ...
    if (status === 'error' && metrics && typeof metrics.memoryUsage === 'number' && metrics.memoryUsage > 0.90 && currentAgent) {
      ...
      const alertData = {
        ...
        status: 'new' as 'new' | 'acknowledged' | 'resolved' | 'ignored',
        metadata: {
          agentId: agentId,
