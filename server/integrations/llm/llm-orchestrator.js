...
export class LLMOrchestrator {
     ...
     */
    setMetricsPersistence(persistence) {
        this.metricsPersistence = persistence;
    }
    /**
     * Initialize a provider with credentials
     */
    async initializeProvider(providerType, credentials) {
     ...
     */
    isProviderInitialized(providerType) {
        const provider = this.providers.get(providerType);
     ...
     */
    selectModelForAnalysis(analysisType, preferredProvider = ProviderType.AUTO, estimatedTokens = 4000, requireLowLatency = false, xPreferredModel) {
        ...
        if (estimatedTokens <= 8000 && requireLowLatency) {
            ...
            const smallModels = eligibleModels.filter(({ model }) => model.maxContextTokens <= 16000 && model.averageLatencyMs < 1500);
            ...
            const mediumModels = eligibleModels.filter(({ model }) => model.maxContextTokens >= estimatedTokens && model.maxContextTokens <= 200000);
            ...
            const largeModels = eligibleModels.filter(({ model }) => model.maxContextTokens >= estimatedTokens);
    ...
    async generateCompletion(options) {
        const { provider: specifiedProvider = ProviderType.AUTO, estimatedTokens = 4000, requireLowLatency = false, xPreferredModel, ...completionOptions } = options;
        ...
        if (completionOptions.modelId && specifiedProvider === ProviderType.AUTO) {
            for (const [providerType, provider] of this.providers.entries()) {
                if (provider.isInitialized() &&
                    ...
                    // Use this provider
                    const result = await this.callProvider(providerType, completionOptions);
        ...
        const { provider, modelId } = this.selectModelForAnalysis(AnalysisType.ALERT_ANALYSIS, // default analysis type
        specifiedProvider, estimatedTokens, requireLowLatency, xPreferredModel);
     ...
     */
    async callProvider(providerType, options) {
        const provider = this.providers.get(providerType);
        if (!provider) {
            throw new Error(`Provider ${providerType} not found`);
        }
