...
import { createTaxiiConnector } from "./integrations/taxii-connector";
...
advancedRouter.post("/parse-structured-data", isAuthenticated, async (req, res) => {
    try {
        ...
        const formatType = format ?
            (DataFormat[format.toUpperCase()] || DataFormat.UNKNOWN) :
            undefined;
    ...
    try {
        ...
        if (options.generateIncidents) {
            ...
            for (const suggestion of incidentSuggestions) {
                try {
                    const incident = await storage.createIncident(suggestion);
                    if (incident) {
...
taxiiRouter.post("/import-intelligence", isAuthenticated, async (req, res) => {
    try {
        ...
        const taxiiConnector = createTaxiiConnector(connector.configuration, connectorId);
