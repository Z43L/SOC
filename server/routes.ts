...
app.use("/downloads", express.static(path.join(process.cwd(), "public", "downloads")));
  ...
  apiRouter.get("/dashboard", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      const openIncidents = incidentsInRange.filter(incident => 
        incident.status !== 'closed' && incident.status !== 'resolved'
      ).length;
      ...
      const mitreTactics = alertsInRange.reduce((acc: any, alert) => {
        if (alert.mitreAttack && alert.mitreAttack.tactics) {
          alert.mitreAttack.tactics.forEach((tactic: string) => {
